...
def RK4(f,fp,funcfp,dt,dx,t,BC,diff_type):
	k1 = np.zeros(len(f))
	k2 = np.zeros(len(f))
	k3 = np.zeros(len(f))
	k4 = np.zeros(len(f))
	ft = np.zeros(len(f))
	funcfp(f,fp,dx,t,BC,diff_type)
	for i in range(0,len(f)):
		k1[i]=fp[i]*dt
	for i in range(0,len(f)):
		ft[i]=f[i]+0.5*k1[i]
	funcfp(ft,fp,dx,t+0.5*dt,BC,diff_type)
	for i in range(0,len(f)):
		k2[i]=fp[i]*dt
	for i in range(0,len(f)):
		ft[i]=f[i]+0.5*k2[i]
	...
	for i in range(0,len(f)):
		ft[i]=f[i]+k3[i]
	funcfp(ft,fp,dx,t+dt,BC,diff_type)
	for i in range(0,len(f)):
		k4[i]=fp[i]*dt
	for i in range(0,len(f)):
		f[i]=f[i]+(k1[i]+2.*k2[i]+2.*k3[i]+k4[i])/6.
def PC_Adam(f,fp,funcfp,dt,f1,f2,f3,dx,t,BC,diff_type):
	ft = np.zeros(len(f))
	funcfp(f,fp,dx,t,BC,diff_type)
	for i in range(0,len(f)):
